		...
		if x == 1:																# Strings of length 1
			for password in passwords:
				for x in xrange(0,100):
					for char1 in chars:
						string = password + char1 + str(format(x, '02d'))
		...
		if x == 2:																# Strings of length 2
			for password in passwords:
				for x in xrange(0,100):
					for char1 in chars:
						for char2 in chars:
							string = password + char1 + char2 + str(format(x, '02d'))
							check_result = check(string, data)
							if(check_result[0]):
								print_out(OUTPUT_FILENAME, check_result[1], check_result[2])
								results[check_result[1]] = check_result[2]
			...
			for password in passwords:	
				for x in xrange(0,100):
					for char1 in chars:
						for char2 in chars:
							for char3 in chars:
								string = password + char1 + char2 + char3 + str(format(x, '02d'))
								check_result = check(string, data)
								if(check_result[0]):
									print_out(OUTPUT_FILENAME, check_result[1], check_result[2])
									results[check_result[1]] = check_result[2]
...
def make_keywords_2(HASH_FILENAME, CHARACTERS_FILENAME, OUTPUT_FILENAME, PASSWORD_FILE):
	...
	for x in xrange(1,6):
		if x == 1:																# Strings of length 1
			for password in passwords:
				for char1 in chars:
					string = char1 + password + str(format(x, '02d'))
					check_result = check(string, data)
					if(check_result[0]):
						print_out(OUTPUT_FILENAME, check_result[1], check_result[2])
						results[check_result[1]] = check_result[2]
		if x == 2:																# Strings of length 2
			for password in passwords:
				for char1 in chars:
					for char2 in chars:
						string = char1 + char2 + password + str(format(x, '02d'))
						check_result = check(string, data)
						if(check_result[0]):
							print_out(OUTPUT_FILENAME, check_result[1], check_result[2])
							results[check_result[1]] = check_result[2]
		if x == 3:																# Strings of length 3
			for password in passwords:	
				for char1 in chars:
					for char2 in chars:
						for char3 in chars:
							string = char1 + char2 + char3 + password + str(format(x, '02d'))
							check_result = check(string, data)
							if(check_result[0]):
								print_out(OUTPUT_FILENAME, check_result[1], check_result[2])
								results[check_result[1]] = check_result[2]
